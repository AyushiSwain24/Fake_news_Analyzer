...
if not os.path.exists("fakereal_training_data.csv"):
    print("❌ Error: fakereal_training_data.csv not found!")
...
try:
    ...
    if len(df) == 0:
        print("❌ Error: No data in fakereal_training_data.csv")
        sys.exit(1)
    
    if 'text' not in df.columns or 'label' not in df.columns:
        print("❌ Error: Required columns 'text' and 'label' not found")
        sys.exit(1)
    ...
    model_fake = LogisticRegression(max_iter=1000, random_state=42)
    ...
    print("✅ Saved model_fake.pkl and vectorizer_fake.pkl")
    
except Exception as e:
    print(f"❌ Error training fake/real model: {e}")
    sys.exit(1)

# --- MODEL 2: TOPIC CLASSIFIER (Upgraded) ---
print("\n--- Training Topic Model (Logistic Regression) ---")

if not os.path.exists("topic_training_data.csv"):
    print("⚠️  Warning: topic_training_data.csv not found - topic model training skipped")
    sys.exit(0)
...
try:
    ...
    if len(df_topic) == 0:
        print("❌ Error: No data in topic_training_data.csv")
        sys.exit(1)
    
    if 'text' not in df_topic.columns or 'topic' not in df_topic.columns:
        print("❌ Error: Required columns 'text' and 'topic' not found")
        sys.exit(1)
    ...
    model_topic = LogisticRegression(max_iter=1000, random_state=42)
    ...
    print("✅ Saved model_topic.pkl and vectorizer_topic.pkl")
